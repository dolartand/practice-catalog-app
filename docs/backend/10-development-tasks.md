...
- [x] Админский список товаров: `GET /admin/products` с фильтром `status=ACTIVE|INACTIVE|DELETED` (+ `q`, `categoryId`, сортировки, пагинация) — видит скрытые и удалённые товары
- [x] Админская карточка товара: `GET /admin/products/{id}` — любой статус, флаги `isActive`/`deletedAt`; просмотр перед реактивацией (`PATCH isActive=true`)
- [x] Полное дерево категорий: `GET /admin/categories` — включая неактивные и soft-deleted, флаги `isActive`/`deletedAt`/`activeProductCount`; публичное дерево — только активные (CTE отфильтровывает `is_active=false`)
- [x] Снятие `@SQLRestriction("deleted_at IS NULL")` c Product/Category — фильтрация удаления явная в каждом запросе (иначе админ не видит soft-deleted записи)
...
- [x] Админ: `GET /admin/orders` (фильтр по статусу) — элементы-карточки с `itemCount` без позиций; `GET /admin/orders/{id}` — детали: адрес, комментарий, позиции (`items[]`), история статусов (`statusHistory[]`)
- [x] Тесты: конкурентное списание остатка (два параллельных заказа), отмена возвращает остатки, идемпотентность оформления и переходов; детали заказа оператора (позиции + история)
...
- [x] Модерация: `GET /admin/reviews` (фильтр `isModerated`) — обогащённые данные для модератора: email/имя пользователя, название и артикул товара (batch-загрузка, без N+1); `PATCH /admin/reviews/{id}/moderation`; публичный список — только одобренные
